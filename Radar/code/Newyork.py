...
base_output_dir = os.path.join(os.path.dirname(__file__), "..", "refcavg")
base_output_dir = os.path.abspath(base_output_dir)
os.makedirs(base_output_dir, exist_ok=True)  # Ensure the Radar\refcavg directory exists

# Clear all PNG files in the Radar directory before running
for fname in os.listdir(base_output_dir):
    if fname.endswith(".png"):
        try:
            os.remove(os.path.join(base_output_dir, fname))
...
for site in stations:
    ...
    try:
        ...
        plt.title(
            f"NEXRAD Reflectivity {site} {timeStr_est} EST",
            fontsize=18,
            fontweight='bold',
            color='darkblue',
        )
        ...
        output_file = os.path.join(base_output_dir, f"NEXRAD_Reflectivity_{site}_{timeStr_est}.png")
