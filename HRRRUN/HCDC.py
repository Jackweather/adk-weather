...
for folder in [
    os.path.join(BASE_DIR, "Hrrr", "static", "HCDC", "grib_files"),
    os.path.join(BASE_DIR, "Hrrr", "static", "HCDC")
]:
...
hcdc_dir = os.path.join(output_dir, "static", "HCDC")
grib_dir = os.path.join(hcdc_dir, "grib_files")
os.makedirs(grib_dir, exist_ok=True)
os.makedirs(hcdc_dir, exist_ok=True)
...
# HCDC variable and colormap
variable_hcdc = "HCDC"
def custom_hcdc_colormap():
...
def download_file(hour_str, step):
    ...
    url_hcdc = (f"{base_url}?dir=%2Fhrrr.{date_str}%2Fconus&file={file_name}"
                f"&var_{variable_hcdc}=on&lev_high_cloud_layer=on")
    response = requests.get(url_hcdc, stream=True)
...
def generate_clean_png(file_path, step):
    ...
    # Use 'hcc' as the variable name for High Cloud Cover
    hcc = ds['hcc'].where((ds['hcc'] >= 0) & (ds['hcc'] <= 100))
    lats = ds['latitude']
    lons = ds['longitude']
    fig = plt.figure(figsize=(10, 7), dpi=850)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_extent([-126, -69, 24, 50], crs=ccrs.PlateCarree())
    # Create an alpha mask: 0 where hcc==0, 1 elsewhere
    alpha_mask = (hcc.squeeze() != 0).astype(float)
    mesh = ax.pcolormesh(
        lons, lats, hcc.squeeze(),
        cmap=cmap, norm=norm, transform=ccrs.PlateCarree(), shading='auto', alpha=alpha_mask
    )
    ax.set_axis_off()
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
    png_path = os.path.join(hcdc_dir, f"HCDC_{step:02d}.png")
