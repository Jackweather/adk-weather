...
# --- Clean up old files in grib_files, pngs, and tot_precip directories ---
for folder in [
    os.path.join(BASE_DIR, "NBM", "static", "tot_precip", "grib_files"),
    os.path.join(BASE_DIR, "NBM", "static", "pngs"),
    os.path.join(BASE_DIR, "NBM", "static", "tot_precip")
]:
...
tot_precip_dir = os.path.join(output_dir, "static", "tot_precip")
grib_dir = os.path.join(tot_precip_dir, "grib_files")
png_dir = tot_precip_dir
...
# Custom colormap and levels for total precipitation (inches)
precip_breaks = [
...
precip_colors = [
    "#ffffff",  # 0
    "#e3f2fd",  # 0.01 very light blue
    "#bbdefb",  # 0.05 light blue
    "#90caf9",  # 0.1
    "#64b5f6",  # 0.15
    "#42a5f5",  # 0.2
    "#2196f3",  # 0.3
    ...
    "#1976d2",  # 0.5
    "#1565c0",  # 0.75
    "#0d47a1",  # 1 (color change, deepest blue)
    "#43a047",  # 1.25 (first green)
    "#388e3c",  # 1.5
    "#2e7d32",  # 1.75
    "#fbc02d",  # 2 (color change, yellow)
    "#f9a825",  # 2.5
    "#f57c00",  # 3 (color change, orange)
    "#ef6c00",  # 3.5
    "#e65100",  # 4
    "#e53935",  # 4.5
    "#b71c1c",  # 5
...
def download_file(hour_str, step):
    ...
    url_apcp = (f"{base_url}?dir=%2Fblend.{date_str}%2F{hour_str}%2Fcore&file={file_name}"
                f"&var_{variable_apcp}=on&lev_surface=on")
...
def generate_clean_png(file_path, step):
    ...
    # Use 'apcp' for NBM total precipitation (kg/m^2, which is mm)
    data = ds['tp'].values  # units: kg/m^2 == mm
    data_in = data / 25.4  # Convert mm to inches
    ...
    if 'latitude' in ds and 'longitude' in ds:
        ...
        mesh = ax.pcolormesh(
            lons_plot, lats, data_in.squeeze(),
            cmap=precip_cmap,
            norm=precip_norm,
            shading='auto',
        ...
        for stn_id, stn_name, stn_lat, stn_lon in NY_ASOS_STATIONS:
            ...
            # Plot the precipitation value as white text with black outline
            txt = ax.text(
    ...
    png_path = os.path.join(png_dir, f"totprecip_{step:03d}.png")
