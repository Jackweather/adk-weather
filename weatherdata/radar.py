...
# Explicitly set the directory to weatherdata/radar
BASE_DIR = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
IMG_DIR = os.path.join(BASE_DIR, "weatherdata", "radar")
...
OUT_HEIGHT = 8192   # Increased for higher detail

# Tile settings
TILE_SIZE = 256
ZOOM = 6  # Increased zoom for higher resolution
...
def fetch_tile(z, x, y):
    url = f"http://mesonet.agron.iastate.edu/cache/tile.py/1.0.0/nexrad-n0r/{z}/{x}/{y}.png"
...
def save_conus_snapshot():
    ...
    print(f"Saved CONUS radar snapshot as {filepath} ({OUT_WIDTH}x{OUT_HEIGHT})")
