...
# --- Clean up old files in grib_files and wind directories ---
for folder in [
    os.path.join(BASE_DIR, "NBM", "static", "wind", "grib_files"),
    os.path.join(BASE_DIR, "NBM", "static", "wind")
]:
...
wind_dir = os.path.join(output_dir, "static", "wind")
...
wind_colors = [
    ...
    "#000000",  # black
    "#808080",  # gray
    "#404040",  # extra color to ensure ncolors = 23 for 23 bins
]
wind_bounds = [
    0, 4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60, 64, 68, 72, 76, 80, 84, 88, 92
]
cmap = ListedColormap(wind_colors)
norm = BoundaryNorm(wind_bounds, cmap.N)

# Station list (reuse from gust)
GUST_STATIONS = [
...
def download_file(hour_str, step):
    ...
    url_wind = (f"{base_url}?dir=%2Fblend.{date_str}%2F{hour_str}%2Fcore&file={file_name}"
                f"&var_{variable_wind}=on&lev_{level}=on")
...
def generate_clean_png(file_path, step):
    ...
    wind = ds['si10'] * 2.23694  # m/s to mph
    wind = wind.where((wind >= 0) & (wind <= 120))
    ...
    mesh = ax.pcolormesh(
        lons_plot, lats, wind.squeeze(),
        cmap=cmap,
        norm=norm,
        shading='auto',
        transform=ccrs.PlateCarree()
    )
    # Plot wind values at station locations
    ...
    png_path = os.path.join(png_dir, f"wind_{step:03d}.png")
