...
# --- Clean old files ---
for folder in [
    os.path.join(BASE_DIR, "Hrrr", "static", "lighting", "grib_files"),
    os.path.join(BASE_DIR, "Hrrr", "static", "lighting")
]:
...
# Directories
output_dir = os.path.join(BASE_DIR, "Hrrr", "static", "lighting")
...
if __name__ == "__main__":
    with concurrent.futures.ProcessPoolExecutor(max_workers=3) as executor:
        results = list(executor.map(process_step, range(0, 49)))
    total_flashes_all_steps = sum(f for f in results if f is not None)
