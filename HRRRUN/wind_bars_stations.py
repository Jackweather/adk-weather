...
wind_dir = os.path.join(output_dir, "static", "wind_bars_station")
...
def generate_wind_barbs_png(file_path, step, skip=20, max_barbs=1500, min_dist_deg=0.5):
    ...
    try:
        ...
        # --- Debug output for variable shapes and types ---
        print(f"[DEBUG] Step {step} - file: {file_path}")
        print(f"  u10: shape={getattr(u, 'shape', None)}, ndim={getattr(u, 'ndim', None)}, type={type(u)}")
        print(f"  v10: shape={getattr(v, 'shape', None)}, ndim={getattr(v, 'ndim', None)}, type={type(v)}")
        print(f"  lats: shape={getattr(lats, 'shape', None)}, ndim={getattr(lats, 'ndim', None)}, type={type(lats)}")
        print(f"  lons: shape={getattr(lons, 'shape', None)}, ndim={getattr(lons, 'ndim', None)}, type={type(lons)}")
        print(f"  ds.dims: {ds.dims}")
        print(f"  ds.coords: {list(ds.coords)}")
        print(f"  ds.data_vars: {list(ds.data_vars)}")

        # --- Check for 2D arrays and print their shapes if not ---
        if not (u.ndim == v.ndim == lats.ndim == lons.ndim == 2):
            print(f"Step {step}: Skipping file {file_path} because u/v/lats/lons are not all 2D arrays.")
        ...
        # --- Plot wind barbs at NY_ASOS station locations only ---
        points = []
        u_vals = []
        v_vals = []
        for stn_id, stn_name, stn_lat, stn_lon in NY_ASOS_STATIONS:
            ...
            u_station = u.values[iy, ix]
            v_station = v.values[iy, ix]
            points.append((stn_lat, stn_lon))
            u_vals.append(u_station)
            v_vals.append(v_station)
        points = np.array(points)
        u_vals = np.array(u_vals)
        v_vals = np.array(v_vals)
        ax.barbs(
            points[:, 1], points[:, 0], u_vals, v_vals,
            length=3, linewidth=0.2, transform=ccrs.PlateCarree()  # Set length to 2 for smaller barbs
        )
