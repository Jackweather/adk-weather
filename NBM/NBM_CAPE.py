...
# --- Clean up old files in grib_files and cape directories ---
for folder in [
    os.path.join(BASE_DIR, "NBM", "static", "cape", "grib_files"),
    os.path.join(BASE_DIR, "NBM", "static", "cape")
]:
...
cape_dir = os.path.join(output_dir, "static", "cape")
...
# CAPE normalization: 0 to 5000 J/kg (adjust as needed)
cape_norm = Normalize(vmin=0, vmax=5000)
...
def download_file(hour_str, step):
    ...
    url_cape = (f"{base_url}?dir=%2Fblend.{date_str}%2F{hour_str}%2Fcore&file={file_name}"
                f"&var_{variable_cape}=on&lev_{level}=on")
    response = requests.get(url_cape, stream=True)
...
def generate_clean_png(file_path, step):
    ...
    # CAPE is in J/kg, no conversion needed
    cape = ds['cape']
    cape = cape.where((cape >= 0) & (cape <= 7000))
    ...
    mesh = ax.pcolormesh(
        lons_plot, lats, cape.squeeze(),
        cmap=cape_cmap,
        norm=cape_norm,
        shading='auto',
        transform=ccrs.PlateCarree()
    )
    # Plot CAPE values at station locations
    ...
    png_path = os.path.join(png_dir, f"cape_{step:03d}.png")
...
print("All NBM CAPE file download and PNG creation tasks complete!")
print("All NBM CAPE file download and PNG creation tasks complete!")
