...
for folder in [
    os.path.join(BASE_DIR, "NBM", "static", "tmp_surface", "grib_files"),
    os.path.join(BASE_DIR, "NBM", "static", "pngs"),
    os.path.join(BASE_DIR, "NBM", "static", "tmp_surface")
]:
...
def download_file(hour_str, step):
    ...
    url_tmp = (f"{base_url}?dir=%2Fblend.{date_str}%2F{hour_str}%2Fcore&file={file_name}"
               f"&var_{variable_tmp}=on&lev_2_m_above_ground=on")  # <-- changed here
...
def generate_clean_png(file_path, step):
    ...
    # Use 't' instead of 't2m' for NBM surface temperature
