...
for folder in [
    os.path.join(BASE_DIR, "GFS", "static", "gfs_total_accum_precip", "grib_files"),
    os.path.join(BASE_DIR, "GFS", "static", "gfs_total_accum_precip")
]:
...
precip_dir = os.path.join(output_dir, "static", "gfs_total_accum_precip")
...
# Function to generate a PNG for total accumulated precip up to this step
def generate_accum_png(accum_data, lats, lons, step):
    ...
    png_path = os.path.join(png_dir, f"gfs_accum_precip_{step:03d}.png")
...
for step in forecast_steps:
    ...
    if 'apcp' in ds.variables:
        ...
        print(f"Neither 'apcp' nor 'tp' found in {grib_file}. Available variables: {list(ds.variables)}")
        continue
