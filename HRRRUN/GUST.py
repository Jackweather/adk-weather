...
for folder in [
    os.path.join(BASE_DIR, "Hrrr", "static", "GUST", "grib_files"),
    os.path.join(BASE_DIR, "Hrrr", "static", "pngs"),
    os.path.join(BASE_DIR, "Hrrr", "static", "GUST")
]:
...
# Gust variable and colormap (in mph)
variable_gust = "GUST"
# New color scale: blue (light) to red (extreme) to gray (out of range)
colors = [
...
def download_file(hour_str, step):
    ...
    url_gust = (f"{base_url}?dir=%2Fhrrr.{date_str}%2Fconus&file={file_name}"
                f"&var_{variable_gust}=on&lev_surface=on")
...
# Station list (copied from PWAT.py, can be renamed as needed)
GUST_STATIONS = [
...
def generate_clean_png(file_path, step):
    ...
    # Convert gust from knots to mph
    gust = ds['gust'] * 1.15078
    ...
    png_path = os.path.join(gust_dir, f"GUST_{step:02d}.png")
