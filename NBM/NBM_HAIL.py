...
# --- Clean up old files in grib_files and hail directories ---
for folder in [
    os.path.join(BASE_DIR, "NBM", "static", "hail", "grib_files"),
    os.path.join(BASE_DIR, "NBM", "static", "hail")
]:
...
hail_dir = os.path.join(output_dir, "static", "hail")
...
variable_hail = "HAILPROB"
level = "surface"

# Hail probability color scale (0-100%)
colors = [
...
def download_file(hour_str, step):
    ...
    url_hail = (f"{base_url}?dir=%2Fblend.{date_str}%2F{hour_str}%2Fcore&file={file_name}"
                f"&var_{variable_hail}=on&lev_{level}=on")
    response = requests.get(url_hail, stream=True)
...
def generate_clean_png(file_path, step):
    ...
    hailprob = ds['hailprob']  # 0-100 (%)
    hailprob = hailprob.where((hailprob >= 0) & (hailprob <= 100))
    ...
    mesh = ax.pcolormesh(
        lons_plot, lats, hailprob.squeeze(),
        cmap=cmap,
        norm=norm,
        shading='auto',
        transform=ccrs.PlateCarree()
    )
    # Plot hail probability values at station locations
    for stn_id, stn_name, stn_lat, stn_lon in GUST_STATIONS:
        ...
        hail_val = hailprob.squeeze()[iy, ix]
        txt = ax.text(
            stn_lon, stn_lat, f"{hail_val:.0f}",
    ...
    png_path = os.path.join(png_dir, f"hail_{step:03d}.png")
...
print("All NBM HAIL GRIB file download and PNG creation tasks complete!")
