...
for folder in [
    os.path.join(BASE_DIR, "GFS", "static", "gfs_total_24_hour", "grib_files"),
    os.path.join(BASE_DIR, "GFS", "static", "gfs_total_24_hour")
]:
...
precip_dir = os.path.join(output_dir, "static", "gfs_total_24_hour")
...
# Function to download GRIB files (GFS APCP)
...
def generate_clean_png(file_path, prev_file_path, step):
    ds = xr.open_dataset(file_path, engine="cfgrib")
    # Try both 'apcp' and 'tp' as variable names
    if 'apcp' in ds.variables:
        data = ds['apcp'].values  # mm
    elif 'tp' in ds.variables:
        data = ds['tp'].values  # mm
    else:
        print(f"Neither 'apcp' nor 'tp' found in {file_path}. Available variables: {list(ds.variables)}")
        return None

    if prev_file_path is not None:
        ds_prev = xr.open_dataset(prev_file_path, engine="cfgrib")
        if 'apcp' in ds_prev.variables:
            data_prev = ds_prev['apcp'].values
        elif 'tp' in ds_prev.variables:
            data_prev = ds_prev['tp'].values
        else:
            data_prev = np.zeros_like(data)
        # Ensure shapes match
        if data.shape != data_prev.shape:
            print(f"Shape mismatch for {file_path} and {prev_file_path}: {data.shape} vs {data_prev.shape}")
            # Try to broadcast or skip
            try:
                data_prev = np.broadcast_to(data_prev, data.shape)
            except Exception as e:
                print(f"Could not broadcast previous data: {e}")
                return None
        # Only mask where either is nan
        mask_nan = np.isnan(data) | np.isnan(data_prev)
        data_24h = np.where(mask_nan, np.nan, data - data_prev)
    else:
        data_24h = np.full_like(data, np.nan)

    data_in = data_24h / 25.4  # Convert mm to inches

    # --- Remove random data beams: mask invalid/extreme values ---
    data_in = np.where(
        (np.isnan(data_in)) | (data_in < 0) | (data_in > 50),
        np.nan,
        data_in
    )

    # --- Mask out data outside the desired plotting extent ---
    # Extent: lon -126 to -69, lat 24 to 50
    ...
    if 'latitude' in ds and 'longitude' in ds:
        ...
        # Mask out data outside the specified extent
        mask_extent = (
        ...
        ax.set_extent([extent_left, extent_right, extent_bottom, extent_top], crs=ccrs.PlateCarree())  # <-- Always set this

        # Use contourf for smooth color transitions
        contour = ax.contourf(
        ...
        for stn_id, stn_name, stn_lat, stn_lon in NY_ASOS_STATIONS:
            ...
            # Use 2D arrays for distance calculation
        ...
        # Mask out data outside CONUS for imshow (if possible)
        contour = ax.imshow(
            np.where(
    ...
    png_path = os.path.join(png_dir, f"gfs24hrprecip_{step:03d}.png")
    plt.savefig(png_path, bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close(fig)
    print(f"Generated clean PNG: {png_path}")
    return png_path

# Function to generate a clean PNG from GRIB files (sum of 4 files)
def generate_clean_png_sum(file_paths, step):
    # file_paths: list of 4 file paths (6h increments)
    data_sum = None
    lats = lons = None
    for fp in file_paths:
        ...
        if 'apcp' in ds.variables:
            ...
            print(f"Neither 'apcp' nor 'tp' found in {fp}. Available variables: {list(ds.variables)}")
            return None
        if data_sum is None:
            data_sum = np.zeros_like(data)
            # Save lats/lons for plotting
            if 'latitude' in ds and 'longitude' in ds:
                lats = ds['latitude'].values
                lons = ds['longitude'].values
        # Only sum valid data
        data_sum = data_sum + np.where(np.isnan(data), 0, data)
    data_in = data_sum / 25.4  # mm to inches
...
# Download all 6-hourly steps (6, 12, ..., 384)
forecast_steps = list(range(6, 385, 6))
grib_files = {}
png_files = []

# Download all needed GRIB files for 6-hour steps
for step in forecast_steps:
    grib_file = download_file(hour_str, step)
    grib_files[step] = grib_file

# For each 24-hour period, sum the four 6-hourly increments and plot the 24-hour total
# E.g., for step=24, sum steps 6,12,18,24; for step=48, sum 30,36,42,48, etc.
for step in range(24, 385, 24):
    steps_6h = [step - 18, step - 12, step - 6, step]
    file_paths = [grib_files.get(s) for s in steps_6h]
    if all(file_paths):
        png_file = generate_clean_png_sum(file_paths, step)
...
# --- Clean up old files in grib_files and output directory ---
for folder in [
    os.path.join(BASE_DIR, "GFS", "static", "gfs_total_precip", "grib_files"),
    os.path.join(BASE_DIR, "GFS", "static", "gfs_total_precip")
]:
    if os.path.exists(folder):
        for f in os.listdir(folder):
            file_path = os.path.join(folder, f)
...
# Directories
output_dir = os.path.join(BASE_DIR, "GFS")
precip_dir = os.path.join(output_dir, "static", "gfs_total_precip")
grib_dir = os.path.join(precip_dir, "grib_files")
png_dir = precip_dir
os.makedirs(grib_dir, exist_ok=True)
os.makedirs(png_dir, exist_ok=True)
...
grib_files = []
png_files = []
# Only include steps from 6 onward, skip 0 (f000)
forecast_steps = list(range(6, 385, 6))
