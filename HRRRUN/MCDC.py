...
for folder in [
    os.path.join(BASE_DIR, "Hrrr", "static", "MCDC", "grib_files"),
    os.path.join(BASE_DIR, "Hrrr", "static", "pngs"),
    os.path.join(BASE_DIR, "Hrrr", "static", "MCDC")
]:
...
mcdc_dir = os.path.join(output_dir, "static", "MCDC")
grib_dir = os.path.join(mcdc_dir, "grib_files")
...
# MCDC variable and colormap
variable_mcdc = "MCDC"
def custom_mcdc_colormap():
...
def download_file(hour_str, step):
    ...
    url_mcdc = (f"{base_url}?dir=%2Fhrrr.{date_str}%2Fconus&file={file_name}"
                f"&var_{variable_mcdc}=on&lev_middle_cloud_layer=on")
    response = requests.get(url_mcdc, stream=True)
...
def generate_clean_png(file_path, step):
    ...
    # Use 'mcc' as the variable name for Medium Cloud Cover
    mcc = ds['mcc'].where((ds['mcc'] >= 0) & (ds['mcc'] <= 100))
    lats = ds['latitude']
    lons = ds['longitude']
    fig = plt.figure(figsize=(10, 7), dpi=850)
    ax = plt.axes(projection=ccrs.PlateCarree())
    ax.set_extent([-126, -69, 24, 50], crs=ccrs.PlateCarree())
    # Create an alpha mask: 0 where mcc==0, 1 elsewhere
    alpha_mask = (mcc.squeeze() != 0).astype(float)
    mesh = ax.pcolormesh(
        lons, lats, mcc.squeeze(),
        cmap=cmap, norm=norm, transform=ccrs.PlateCarree(), shading='auto', alpha=alpha_mask
    )
    ax.set_axis_off()
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
    png_path = os.path.join(mcdc_dir, f"MCDC_{step:02d}.png")
