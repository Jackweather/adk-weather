...
for folder in [
    os.path.join(BASE_DIR, "Hrrr", "static", "PWAT", "grib_files"),
    os.path.join(BASE_DIR, "Hrrr", "static", "PWAT")
]:
...
pwat_dir = os.path.join(output_dir, "static", "PWAT")
grib_dir = os.path.join(pwat_dir, "grib_files")
os.makedirs(grib_dir, exist_ok=True)
os.makedirs(pwat_dir, exist_ok=True)
...
# PWAT variable and colormap (now in inches)
variable_pwat = "PWAT"
colors = [
    "#f7fbff", "#deebf7", "#c6dbef", "#9ecae1", "#6baed6",
    "#4292c6", "#2171b5", "#08519c", "#08306b", "#00441b",
    "#238b45", "#41ae76", "#66c2a4", "#99d8c9", "#ccece6", "#e5f5f9"
]
# Bounds in inches (converted from mm)
bounds = [x / 25.4 for x in [0, 5, 10, 15, 20, 25, 30, 35, 40, 45, 50, 55, 60, 65, 70, 75]]
...
def download_file(hour_str, step):
    ...
    url_pwat = (f"{base_url}?dir=%2Fhrrr.{date_str}%2Fconus&file={file_name}"
                f"&var_{variable_pwat}=on&lev_entire_atmosphere_%28considered_as_a_single_layer%29=on")
    response = requests.get(url_pwat, stream=True)
...
def generate_clean_png(file_path, step):
    ...
    # The variable name may be lowercase or uppercase, check both
    pwat = ds.get('pwat') if 'pwat' in ds else ds.get('PWAT')
    if pwat is None:
        print(f"PWAT variable not found in {file_path}")
        return None
    pwat_in = pwat.where((pwat >= 0) & (pwat <= 75)) / 25.4  # Convert to inches for plotting and masking
    lats = ds['latitude']
    lons = ds['longitude']
    ...
    contour = ax.contourf(
        lons, lats, pwat_in.squeeze(),
        levels=bounds, cmap=cmap, norm=norm, transform=ccrs.PlateCarree(), extend='max'
    )
    # Plot PWAT values at NY_ASOS stations (after mesh, with high zorder)
    for stn_id, stn_name, stn_lat, stn_lon in NY_ASOS_STATIONS:
        ...
        if lats_np.ndim == 2 and lons_np.ndim == 2:
        ...
        pwat_val_in = pwat_in.squeeze()[iy, ix]
        txt = ax.text(
            stn_lon, stn_lat, f"{pwat_val_in:.2f}",
    ...
    png_path = os.path.join(pwat_dir, f"PWAT_{step:02d}.png")
