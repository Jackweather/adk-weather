...
# --- Clean up old files in grib_files and tstm directories ---
for folder in [
    os.path.join(BASE_DIR, "NBM", "static", "tstm", "grib_files"),
    os.path.join(BASE_DIR, "NBM", "static", "tstm")
]:
...
tstm_dir = os.path.join(output_dir, "static", "tstm")
grib_dir = os.path.join(tstm_dir, "grib_files")
png_dir = tstm_dir
...
# Thunderstorm probability color scale (0-100%)
colors = [
    (1, 1, 1, 0),    # 0% (clear/transparent)
    "#e0f7fa",        # 0-5% (very light cyan)
    "#b2ebf2",        # 5-10% (light cyan)
    "#4dd0e1",        # 10-20% (cyan)
    "#388e3c",        # 20-30% (green)
    "#fff176",        # 30-40% (yellow)
    "#ffd54f",        # 40-50% (orange)
    "#ff7043",        # 50-60% (red-orange)
    "#d32f2f",        # 60-70% (red)
    "#7b1fa2",        # 70-80% (purple)
    "#616161",        # 80-100% (gray)
]
...
def download_file(hour_str, step):
    ...
    url_tstm = (f"{base_url}?dir=%2Fblend.{date_str}%2F{hour_str}%2Fcore&file={file_name}"
                f"&var_{variable_tstm}=on&lev_{level}=on")
    response = requests.get(url_tstm, stream=True)
...
def generate_clean_png(file_path, step):
    ...
    tstmprob = ds['tstm']  # 0-100 (%)
    tstmprob = tstmprob.where((tstmprob >= 0) & (tstmprob <= 100))
    ...
    mesh = ax.pcolormesh(
        lons_plot, lats, tstmprob.squeeze(),
        cmap=cmap,
        norm=norm,
        shading='auto',
        transform=ccrs.PlateCarree()
    )
    # Plot thunderstorm probability values at station locations
    for stn_id, stn_name, stn_lat, stn_lon in GUST_STATIONS:
        ...
        tstm_val = tstmprob.squeeze()[iy, ix]
        txt = ax.text(
            stn_lon, stn_lat, f"{tstm_val:.0f}",
    ...
    png_path = os.path.join(png_dir, f"tstm_{step:03d}.png")
...
print("All NBM TSTM GRIB file download and PNG creation tasks complete!")
